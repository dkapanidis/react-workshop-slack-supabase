...
function WorkspaceButton({ title, shortcut }: Props) {
  ...
  return (
    <div data-tip data-for={`${shortcut}`} className="cursor-pointer mb-5">
      <NavLink className="bg-gray-500 h-10 w-10 flex items-center justify-center text-black text-2xl font-semibold rounded-lg mb-1 overflow-hidden hover:ring-4 ring-offset-4 ring-offset-gray-900 hover:ring-gray-700" activeClassName="ring-4 ring-gray-300" to={`/workspace/${title.toLowerCase()}`}>
        {title[0]}
      </NavLink>
